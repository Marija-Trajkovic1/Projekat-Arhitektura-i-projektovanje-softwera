...
const EmployerFollowing = ({ onEmployersChange }) => {
  ...
  useEffect(() => {
    ...
    connection.on("NewJobPosted", handleNewEmployer);

    return () => {
      connection.off("NewEmployerAdded", handleNewEmployer);
    };
  }, [connection]);
        ...
        {employersList.map((employer) => {
          ...
          return (
            ...
            >
              ...
              <p className={pStyle}>
                {employer.city}, {employer.street}, {employer.homeNumber}
              </p>
              <button
                onClick={() => clickOnFollow(employer.id)}
                disabled={isLoading}
